    ...
    .prompt([
      {
        ...
        name: "keys",
        message,
        choices: getSortedPackages(Array.from(packages.values())).map((pkg) => ({
...
export async function getPackagesByTarget(target: string | undefined, message: string) {
  ...
  switch (target) {
    case "root": {
      return list.filter((pkg) => pkg.type === "root");
    }
    case "replica": {
    ...
    case "shared": {
      return list.filter((pkg) => pkg.type === "shared");
    }
    case "all": {
      return list;
    }
 ...
 * @returns Package.
 */
export function getPackage(uri: string) {
  try {
    const pkg = fs.readFileSync(path.resolve(uri), "utf-8");
    if (!pkg) {
 ...
 * @returns Packages.
 */
export function getSortedPackages(packages: Package[], order = ["root", "shared", "replica", "module"]): Package[] {
